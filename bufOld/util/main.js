...
function updateBoard(grids) {
    ...
    if (talleyScores == 1) {
        for (let user of users)
            scores.push(getUserScore(grids[user], dates.length));
    }
    ...
    for (let y = 0; y < users.length; ++y) {
        ...
        for (let x = 0; x < dates.length; ++x) {
            ...
            if (played && x < dates.length - 1)
                document.getElementById(cellID(x, y)).innerHTML = img("walk.jpg");
        }
    }
    ...
    if (talleyScores)
        for (let y = 0; y < users.length; ++y) {
            ...
            if (pastDate || grid[grid.length - 1].date == dates[dates.length - 1]) {
                if (scores[y] > scores[keyUser] || y == keyUser)
                    document.getElementById(cellID(dates.length, y)).innerHTML = scores[y].toFixed(2);
                else
                    document.getElementById(cellID(dates.length, y)).innerHTML = scores[y].toFixed(2) + "<BR>" + img("grave.jpg");
...
function getUserScore(grids, length) {
    let r = 0;
    for (let grid of grids)
        r += score(decode(grid.grid));
    return r / grids.length * length;
}
...
function uniformColor(grid) {
    ...
    r = r.replaceAll("⬛", "<span class = 'boxB'></span>");
    return r;
}
